...
const mockedUpdateFoodVariant = jest.mocked(updateFoodVariant);

describe('persistExternalVariants', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockedCreateFoodVariant.mockResolvedValue({ id: 'created-variant' } as any);
    mockedUpdateFoodVariant.mockResolvedValue({ id: 'updated-variant' } as any);
  });
  ...
  it('updates an existing provider variant when Yazio has a richer serving description and weight', async () => {
    mockedFetchFoodVariants.mockResolvedValue([
      {
        id: 'existing-piece',
        food_id: 'food-1',
    ...
    await persistExternalVariants(
      { id: 'food-1', default_variant: { serving_size: 1, serving_unit: 'piece' } },
      [
        {
          serving_size: 1,
          serving_unit: 'piece',
    ...
    expect(mockedUpdateFoodVariant).toHaveBeenCalledTimes(1);
    expect(mockedUpdateFoodVariant).toHaveBeenCalledWith(
      'existing-piece',
      expect.objectContaining({
        food_id: 'food-1',
        serving_size: 1,
