...
- If that native header needs buttons, document the expected `unstable_headerLeftItems` / `unstable_headerRightItems` in `NATIVE_HEADER_ROOT_ROUTE_ITEMS` so missing native buttons fail tests.
